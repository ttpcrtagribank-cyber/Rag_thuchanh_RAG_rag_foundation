...
- Nội quy lao động, quy trình chung cho `Employee`, `Guest`.
...
2. Thiết kế logic gán tag quyền dựa trên tiêu chí sau:
   ...
   - Quy tắc phân bổ:
     * Dựa trên `document_id` hoặc từ khóa trong `text` để phân loại:
       ...
       + Nếu tài liệu liên quan đến "tín dụng", "rủi ro", "hạn mức", "phê duyệt duyệt vay": allowed_roles = ["Admin", "Risk_Officer", "Employee"]
       + Các tài liệu quy định chung khác: allowed_roles = [`Admin`, `HR_Manager`, `Risk_Officer`, `Employee`, `Guest`]
