...
Buổi 17 không xây lại RAG từ đầu. Học viên tái sử dụng dữ liệu và Secure Retrieval đã hình thành ở Buổi 14, đồng thời tận dụng Hybrid Search, Reranking, Metadata và Knowledge Graph từ các buổi trước để bổ sung ba năng lực:
...
```env
# Dữ liệu đầu vào Buổi 14 nằm ở thư mục cùng cấp
SOURCE_SECURE_CSV=../buoi_14/data/processed/chunks_secure.csv
SOURCE_NORMALIZED_CSV=../buoi_14/data/processed/chunks_normalized.csv
...
Folder Buổi 17 nằm cùng cấp với Buổi 14.
...
- .env của buoi_17;
- code SecureRetriever từ Buổi 14 nằm ở đâu và import được không;
- Neo4j chỉ kiểm tra nếu bước sau thực sự dùng.

Chỉ kiểm tra, chưa sửa dữ liệu Buổi 14 và chưa viết lại retriever.
...
# PROMPT 0 — Đọc dữ liệu và code Buổi 14 trước khi làm

```text
Đọc Buổi 14 trước khi làm Buổi 17.
...
Tìm code SecureRetriever mà Buổi 14 đang dùng.
Báo:
- file/module;
- hàm/class chính;
- input role;
...
Sau đó kiểm tra SecureRetriever của Buổi 14:
...
# PROMPT 2 — Secure Retrieval bằng code Buổi 14

```text
Tái sử dụng SecureRetriever của Buổi 14 cho Buổi 17.
...
Tái sử dụng SecureRetriever Buổi 14 thông qua adapter của Buổi 17 nếu cần.
...
Buổi 17 phải đọc dữ liệu Buổi 14 ở chế độ read-only.
