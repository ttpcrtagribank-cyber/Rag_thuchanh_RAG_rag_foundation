...
"""
    ...
    risk_links = "\n".join([f"{idx}. [[{basename_by_id[e['id']]}]]" for idx, e in enumerate([e for e in entities if e["type"] == "RuiRo"], 1)])
...
"""
    ...
    print(f"   - wiki/risks/:    {len(list(risks_dir.glob('*.md')))} trang")
    print(f"   - wiki/controls/: {len(list(controls_dir.glob('*.md')))} trang")
    print(f"   - wiki/events/:   {len(list(events_dir.glob('*.md')))} trang")
