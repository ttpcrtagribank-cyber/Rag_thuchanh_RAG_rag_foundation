...
Vị trí: buoi_17/scripts/audit_checklist_gen.py (hoặc buoi_18/scripts/audit_checklist_gen.py)
...
class AuditChecklistGeneratorEngine:
    ...
    1. Tiếp nhận Domain, Unit và User Role.
    2. Lọc & Truy xuất quy định nội bộ + văn bản NHNN trong phạm vi RBAC.
    3. Đưa Evidence Context sang Gemini LLM để tự động sinh danh mục Checklist kiểm toán.
    4. Đóng gói đầy đủ source_citation, risk_level, recommendation và review_status = "NEEDS_HUMAN_REVIEW".
    ...
    def _llm_generate_checklist_items(
        ...
        """
        Gửi Evidence Context sang Gemini LLM để sinh danh mục Checklist kiểm toán.
        """
        if not relevant_chunks or not self.client:
            # Fallback nếu không có context hoặc client
...
--- YÊU CẦU ĐẦU RA ---
Sinh từ 3 đến 5 mục kiểm tra (Checklist Items) sắc bén, bám sát các điều khoản thực tế trong context trên.
Mỗi mục checklist phải chứa:
1. `item_id`: Mã dạng `CHK_{prefix_code}_01`, `CHK_{prefix_code}_02`, v.v.
2. `domain`: "{domain}"
3. `unit_scope`: "{unit}"
4. `audit_question`: Câu hỏi kiểm toán cụ thể, trực diện đối với đơn vị được kiểm toán.
5. `risk_description`: Mô tả rủi ro tiềm ẩn nếu đơn vị vi phạm hoặc thực hiện không đúng quy định.
6. `risk_level`: Mức độ rủi ro ("HIGH", "MEDIUM", hoặc "LOW").
7. `source_citation`: TRÍCH DẪN CHÍNH XÁC `citation` từ context trên (Bắt buộc dùng citation có sẵn trong context, không tự bịa).
8. `recommendation`: Gợi ý hành động kiểm toán (Ví dụ: Kiểm tra thực địa, đối chiếu chứng từ, soi chiếu log mã hóa, phỏng vấn cán bộ).

--- ĐỊNH DẠNG TRẢ VỀ (JSON ARRAY) ---
Trả về kết quả duy nhất ở dạng JSON Array (không kèm bất kỳ văn bản ngoài json):
[
  {{
...
"""
    ...
    def _build_fallback_items(self, domain: str, unit: str, timestamp: str, request_id: str) -> List[Dict[str, Any]]:
        """
        Khởi tạo fallback items chuẩn nếu không kết nối được LLM.
        """
    ...
    def run_checklist_tests(self) -> List[Dict[str, Any]]:
        ...
        """
        print("[*] Đang khởi chạy AI Audit Checklist Generator Engine cho 2 domain thử nghiệm...")
